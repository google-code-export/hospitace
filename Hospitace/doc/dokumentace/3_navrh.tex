...
V této sekci popíšu základní strukturu aplikace. Struktura aplikace je vygenerovaná pomocí generátor v Ruby on Rails. Proto nepopíšu celou strukturu, ale pouze části aplikace, které jsou nejdůležitější pro její vývoj. Na obrázku \ref{tree:hospitace} je popsaná strukturu nejdůležitějších složek a k nim popis obsahu.
...
\begin{figure}[h]
	\dirtree{%
		...
		.3 helpers/ \DTcomment{pomocné funkce pro generování views}.
		.3 inputs/ \DTcomment{vytvořené vstupní pole pro gem SimpleForm}.
		.3 mailers/ \DTcomment{třídy které generují a odesílají emaily}.
		.3 models/ \DTcomment{modely aplikace}.
		.3 views/ \DTcomment{views aplikace}.		
